...
function printerError(s) {
    ...
    let numOfErrors = splitS.filter(letter => letter > "m");
    return numOfErrors.length + "/" + s.length;
}


//My Explanation:
//first split the string into an array seperated by each letter and asssign it to variable splitS
//use filter on splitS that returns any letter that is greater than "m" (any letter higher than "m" is considered an error) and assign it to numOfErrors
//lastly, return the length of numOfErrors array as the numerator, concatenate "/" to create fraction, and use length of inputted string (s) as the denominator
